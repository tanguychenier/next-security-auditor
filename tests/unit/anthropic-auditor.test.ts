...
import { AnthropicSecurityAuditor } from '../../src/infrastructure/llm/anthropic-auditor.js'
...
const auditor = (text: string) =>
  new AnthropicSecurityAuditor({ apiKey: 'test', fetchImpl: answering(text) })
...
describe('reading what the model answered', () => {
  ...
  it('surfaces an API error instead of pretending the code is clean', async () => {
    const failing = new AnthropicSecurityAuditor({
      apiKey: 'test',
