...
import type { SecurityAuditor } from '../../src/application/ports/security-auditor.js'
...
const suspicious: SecurityAuditor = {
  async suspect(entry) {
...
describe('auditing a Next.js project end to end', () => {
  ...
  it('counts a suspicion the auditor cannot turn into a request as discarded', async () => {
    const speechless: SecurityAuditor = { ...suspicious, async planProof() { return undefined } }
  ...
  it('reports nothing at all when the auditor suspects nothing', async () => {
    const calm: SecurityAuditor = { async suspect() { return [] }, async planProof() { return undefined } }
